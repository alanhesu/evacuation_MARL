...
class raw_env(AECEnv):
    ...
    def step(self, action):
        ...
        self.cur_state[self.agent_selection] = action

        # collect reward if it is the last agent to act
        if self._agent_selector.is_last():
            # rewards for all agents are placed in the .rewards dictionary
            self.rewards[self.agents[0]], self.rewards[self.agents[1]] = REWARD_MAP[(self.cur_state[self.agents[0]], self.cur_state[self.agents[1]])]
            ...
            for i in self.agents:
                self.observations[i] = self.cur_state[self.agents[1 - self.agent_name_mapping[i]]]
        else:
            # necessary so that observe() returns a reasonable observation at all times.
            self.cur_state[self.agents[1 - self.agent_name_mapping[agent]]] = NONE
